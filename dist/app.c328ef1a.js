...
function makeFeeds(newsFeed) {
  for (var i = 0; i < newsFeed.length; i++) {
    newsFeed[i].read = false;
  }
...
function showNews() {
  ...
  for (var i = (store.currentPage - 1) * 10; i < store.currentPage * 10; i++) {
    newsList.push("\n      <div class=\"p-6 ".concat(newsFeed[i].read ? "bg-red-500" : "bg-white", " mt-6 rounded-lg shadow-md transition-colors duration-500 hover:bg-green-100\">\n        <div class=\"flex\">\n          <div class=\"flex-auto\">\n            <a href=\"#/show/").concat(newsFeed[i].id, "\">").concat(newsFeed[i].title, "</a>  \n          </div>\n          <div class=\"text-center text-sm\">\n            <div class=\"w-10 text-white bg-green-300 rounded-lg px-0 py-2\">").concat(newsFeed[i].comments_count, "</div>\n          </div>\n        </div>\n        <div class=\"flex mt-3\">\n          <div class=\"grid grid-cols-3 text-sm text-gray-500\">\n            <div><i class=\"fas fa-user mr-2\"></i>").concat(newsFeed[i].user, "</div>\n            <div><i class=\"fas fa-heart mr-2 ml-4\"></i>").concat(newsFeed[i].points, "</div>\n            <div><i class=\"far fa-clock mr-2 ml-4\"></i>").concat(newsFeed[i].time_ago, "</div>\n          </div>  \n        </div>\n      </div>\n  "));
  }

  container.innerHTML = template.replace("{{__news_feed__}}", newsList.join("")).replace("{{__prev_page__}}", store.currentPage > 1 ? store.currentPage - 1 : 1).replace("{{__next_page__}}", store.currentPage + 1 > newsFeed.length / 10 ? store.currentPage : store.currentPage + 1);
}
...
function showContent() {
  ...
  function makeComment(comments, called) {
    ...
    for (var _i = 0; _i < comments.length; _i++) {
      ...
      if (comments[_i].comments_count > 0) {
...
function router() {
  ...
  if (route === "") showNews();else if (route.indexOf("/page/") >= 0) {
    store.currentPage = Number(route.substr(7));
    showNews();
  } else showContent();
}
